                ...
                <div>
                    {status === "loading" ? (
                        ...
                        <div className="flex flex-col">
                            ...
                            <span className="text-2xl font-bold text-slate-900 dark:text-white">{session.user?.name || session.user?.email?.split('@')[0] || "User"}!</span>
                        </div>
                    ) : (
                        <div className="flex flex-col sm:flex-row sm:items-center gap-3 bg-emerald-50 border border-emerald-100 dark:bg-emerald-900/10 dark:border-emerald-900/20 p-4 rounded-xl">
                            <div className="p-2 bg-emerald-100 dark:bg-emerald-900/30 rounded-lg text-emerald-600 dark:text-emerald-400">
                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2" /><circle cx="9" cy="7" r="4" /><path d="M22 21v-2a4 4 0 0 0-3-3.87" /><path d="M16 3.13a4 4 0 0 1 0 7.75" /></svg>
                            </div>
                            <div>
                                <h3 className="font-semibold text-emerald-900 dark:text-emerald-100">Guest Mode Active</h3>
                                <p className="text-sm text-emerald-700 dark:text-emerald-400">Your data is saved locally. <Link href="/login" className="underline font-medium hover:text-emerald-900 dark:hover:text-emerald-200">Login to sync devices</Link>.</p>
                            </div>
                        </div>
                    )}
                </div>
